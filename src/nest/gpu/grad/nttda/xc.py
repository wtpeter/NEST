...
def _block_loop(ni, mol, grids, nao, deriv, max_memory=None):
    """Yield dense AO blocks in the molecule's original AO order."""
    del max_memory
    opt = ni.gdftopt
    if opt is None:
    ...
    for ao, mask, weights, coords in ni.block_loop(
            sorted_mol, grids, nao, deriv, max_memory=None):
        dense = cp.zeros((len(ao), nao, weights.size), dtype=ao.dtype)
        dense[:, mask] = ao
        dense = opt.unsort_orbitals(dense, axis=[1])
        yield dense.transpose(0, 2, 1), full_mask, weights, coords


def _eval_rho(
        ni, mol, ao, density, mask, xctype, hermi=0, with_lapl=False):
    ao = ao.T if ao.ndim == 2 else ao.transpose(0, 2, 1)
    density = cp.asarray(density)[mask[:, None], mask]
    return ni.eval_rho(
        mol, ao, density, None, xctype, hermi=hermi,
        with_lapl=with_lapl,
    )


def _eval_rho2(
        ni, mol, ao, mo_coeff, mo_occ, mask, xctype, with_lapl=False):
    ao = ao.T if ao.ndim == 2 else ao.transpose(0, 2, 1)
    mo_coeff = cp.asarray(mo_coeff)[mask]
    return ni.eval_rho2(
        mol, ao, mo_coeff, mo_occ, None, xctype,
        with_lapl=with_lapl,
    )


def add_gga_matrix(output, ao, weights):
    weights = cp.asarray(weights, order="C").copy()
    weights[0] *= 0.5
    ao = ao.transpose(0, 2, 1)
    scaled = numint._scale_ao(ao[:4], weights)
    matrix = contract("ig,jg->ij", ao[0], scaled)
    output += matrix + matrix.T


def add_mgga_matrix(mol, output, ao, weights, mask):
    """Accumulate one ordinary meta-GGA feature potential matrix."""
    output += mgga_eval_matrix(mol, ao, weights, mask)


def pair_matrix(mol, ao, tensor):
    output = cp.zeros((mol.nao_nr(), mol.nao_nr()))
    ao = ao.transpose(0, 2, 1)
    for left in range(4):
        scaled = contract(
            "jqg,jg->qg",
            ao[:4],
            cp.asarray(tensor[left], order="C"),
        )
        output += contract("pg,qg->pq", ao[left], scaled)
    return output
...
def _compact_ao_center_derivative(ao, p0, p1, xyz, xctype):
    ...
    delta = cp.empty((4, ao.shape[-2], p1 - p0))
...
def pair_feature_batches(ao, densities):
    ...
    densities = cp.asarray(densities)
    grids = ao.shape[-2]
    features = cp.empty((len(densities), 4, 4, grids))
    contracted = cp.asarray([
        ao[index] @ densities for index in range(4)
    ]).transpose(1, 0, 2, 3)
    for left in range(4):
        for right in range(4):
            features[:, left, right] = cp.einsum(
...
def contract_pair_feature_derivatives(
    ...
    densities = cp.asarray(densities)
    tensor_weights = cp.asarray(tensor_weights)
    value = 0.0
    for left in range(4):
...
def gga_eval_matrix(mol, ao, weights, mask):
    output = cp.zeros((4, mol.nao_nr(), mol.nao_nr()))
    tdrks_grad._gga_eval_mat_(
        mol, output, ao.transpose(0, 2, 1), cp.array(weights, copy=True), mask,
...
def _lda_fref_kref(mf, ao0, mask):
    ...
    fxc, kxc = ni.eval_xc_eff(
        mf.xc, cp.stack((rho0, rho0)), deriv=3, xctype="LDA", spin=1,
    )[2:4]
    fref = 0.5 * (
...
def _lda_matrix(ao0, weights):
    return ao0.T @ (ao0 * cp.asarray(weights)[:, None])


def _unpack_channel_data(channel_data):
    """Return channel data and the spin carried by each target orbital."""
...
def lda_response_terms(
    ...
    nao = mol.nao_nr()
    potentials = {label: cp.zeros((nao, nao)) for label in densities}
    ...
    for ao, mask, weights, _coords in _block_loop(ni,
        ...
        potential_weights = {
            label: cp.zeros_like(weights) for label in densities
        }
        pair_alpha = cp.zeros_like(weights)
        pair_beta = cp.zeros_like(weights)
        ...
        potential_weight_stack = cp.asarray([
...
def lda_nobeta_reference_q(tdobj, p0, max_memory=None):
    ...
    p0 = 0.5 * (cp.asarray(p0) + cp.asarray(p0).T)
    matrix_alpha = cp.zeros((nao, nao))
    matrix_beta = cp.zeros_like(matrix_alpha)
    for ao, mask, weights, _coords in _block_loop(ni,
        ...
        rho_alpha = _eval_rho(ni,
            mol, ao0, density_alpha, mask, "LDA", hermi=1,
            with_lapl=False,
        )
        rho_beta = _eval_rho(ni,
        ...
        fxc_actual = ni.eval_xc_eff(
            mf.xc, cp.stack((rho_alpha, rho_beta)), deriv=2,
            xctype="LDA", spin=1,
        )[2]
        fxc_equal = ni.eval_xc_eff(
            mf.xc, cp.stack((rho0, rho0)), deriv=2,
            xctype="LDA", spin=1,
        )[2]
...
def _gga_fref_kref(mf, rho0):
    fxc, kxc = mf._numint.eval_xc_eff(
        mf.xc, cp.stack((rho0, rho0)), deriv=3, xctype="GGA", spin=1,
    )[2:4]
    fref = 0.5 * (
        fxc[0, :, 0] - fxc[0, :, 1]
        - fxc[1, :, 0] + fxc[1, :, 1]
...
def gga_response_terms(
    ...
    for ao, mask, weights, _coords in _block_loop(ni,
        ...
        ordinary_weights = {
            label: cp.zeros((4, weights.size)) for label in densities
        }
        special_weights = {
            label: cp.zeros((4, 4, weights.size)) for label in pair_labels
        }
        reference_weights_alpha = cp.zeros((4, weights.size))
        reference_weights_beta = cp.zeros_like(reference_weights_alpha)

        for term in terms:
            if term.vref0:
                ordinary_weights[term.target] += term.vref0 * cp.einsum(
                    "xyg,yg->xg", fref, rho[term.source],
                )
                ordinary_weights[term.source] += term.vref0 * cp.einsum(
                    "xyg,xg->yg", fref, rho[term.target],
                )
                pair = term.vref0 * cp.einsum(
                    "xg,yg->xyg", rho[term.target], rho[term.source],
                )
                reference_weights_alpha += cp.einsum(
        ...
        for label in potentials:
            add_gga_matrix(
                potentials[label],
                ao,
                ordinary_weights[label] * weights,
            )
        for label in pair_labels:
            potentials[label] += pair_matrix(
                mol, ao, special_weights[label] * weights,
            )
...
def gga_nobeta_reference_q(tdobj, p0, max_memory=None):
    ...
    density0 = 0.5 * (density_alpha + density_beta)
    p0 = 0.5 * (cp.asarray(p0) + cp.asarray(p0).T)
    matrix_alpha = cp.zeros((mol.nao_nr(), mol.nao_nr()))
    matrix_beta = cp.zeros_like(matrix_alpha)
    for ao, mask, weights, _coords in _block_loop(ni,
        ...
        fxc_equal = ni.eval_xc_eff(
            mf.xc, cp.stack((rho_equal, rho_equal)), deriv=2,
            xctype="GGA", spin=1,
        )[2]
        equal = 0.25 * (
        ...
        matrix_alpha += gga_eval_matrix(
            mol,
            ao,
            cp.einsum("xg,xzg->zg", rho_p, equal - actual_alpha) * weights,
            mask,
        )
        matrix_beta += gga_eval_matrix(
            mol,
            ao,
            cp.einsum("xg,xzg->zg", rho_p, equal - actual_beta) * weights,
            mask,
        )
...
def _mgga_fref_kref(mf, rho0):
    fxc, kxc = mf._numint.eval_xc_eff(
        mf.xc, cp.stack((rho0, rho0)), deriv=3, xctype="MGGA", spin=1,
    )[2:4]
    fref = 0.5 * (
        fxc[0, :, 0] - fxc[0, :, 1]
        - fxc[1, :, 0] + fxc[1, :, 1]
...
def mgga_response_terms(
    ...
    for ao, mask, weights, _coords in _block_loop(ni,
        ...
        ordinary_weights = {
            label: cp.zeros((5, weights.size)) for label in densities
        }
        special_weights = {
            label: cp.zeros((4, 4, weights.size)) for label in pair_labels
        }
        reference_weights_alpha = cp.zeros((5, weights.size))
        reference_weights_beta = cp.zeros_like(reference_weights_alpha)
        ...
        for label in potentials:
            add_mgga_matrix(
                ...
                ordinary_weights[label] * weights, mask,
            )
        for label in pair_labels:
            potentials[label] += pair_matrix(
                mol, ao, special_weights[label] * weights,
